 ...
 * y funcionalidades interactivas embebidas.
 */
class TemplateFormGenerator {
  ...
  renderField(field, currentValue = "") {
    if (!field || !field.type)
      return `<p class="text-red-500 text-xs p-2 bg-red-50 rounded">Error: Campo sin tipo definido.</p>`;
    ...
    // 2. Textarea
      ...
      inputHtml = `<textarea id="${field.id}" name="${field.id}" rows="4"
        ...
        placeholder="${
          field.placeholder || "Escribe aquí los detalles..."
        }" ${requiredAttr}>${currentValue}</textarea>`;
    }
    // 3. URL Compuesta (Link + Texto)
      ...
      inputHtml = `
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 url-group p-1">
             <div class="relative group/url">
               ...
               <div class="absolute inset-y-0 left-0 pl-4 top-6 flex items-center pointer-events-none text-slate-400 group-focus-within/url:text-blue-500 transition-colors">
                 <i class="fas fa-link text-sm"></i>
               </div>
               <input type="url" id="${field.id}_url" 
                      class="${this.inputBaseClass} pl-11 font-mono text-blue-600" 
                      placeholder="https://..." 
             ...
             <div class="relative group/text">
               ...
               <div class="absolute inset-y-0 left-0 pl-4 top-6 flex items-center pointer-events-none text-slate-400 group-focus-within/text:text-slate-600 transition-colors">
                 <i class="fas fa-font text-sm"></i>
               </div>
               <input type="text" id="${field.id}_text" 
                      class="${this.inputBaseClass} pl-11" 
                      placeholder="Ej: Acceso Cliente" 
                      value="${textVal}" />
             </div>
          </div>
        `;
      ...
      inputHtml = `
        <div class="relative group/pass">
            ...
            <input type="password" id="${field.id}" name="${field.id}" 
                class="${this.inputBaseClass} pl-11 pr-12 font-mono tracking-wider" 
                placeholder="••••••••" value="${currentValue}" ${requiredAttr} />
            
            <button type="button" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-secondary cursor-pointer focus:outline-none transition-colors toggle-pass-visibility" tabindex="-1">
                <i class="fas fa-eye"></i>
            </button>
        </div>
        <script>
            // Pequeño script inline para activar el toggle inmediatamente al renderizar
    ...
    // 7. Inputs Estándar
      ...
      // Configuración de iconos y estilos específicos
      let icon = "";
      let extraClasses = "";
      ...
      inputHtml = `
        <div class="relative group/std">
          ...
          <input type="${finalType}" id="${field.id}" name="${field.id}" 
            class="${this.inputBaseClass} ${paddingClass} ${extraClasses}" 
            placeholder="${
              field.placeholder || ""
    ...
    // Layout
    const isFullWidth = ["table", "text", "url", "textarea"].includes(
      field.type
    );
    ...
    return `
      <div class="field-wrapper ${layoutClass} group animate-fade-in-up">
        <label for="${
          field.id
        }" class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2 ml-1 flex items-center justify-between">
  ...
  generateFormHtml(template, data = {}) {
    if (!template || !template.fields)
      return `<div class="p-4 bg-red-50 text-red-600 rounded-lg border border-red-100">Error: Estructura de plantilla inválida.</div>`;
